...
from tf_unet.layers import (bias_variable, conv2d, crop_and_concat,
                            cross_entropy, deconv2d, max_pool,
                            pixel_wise_softmax_2, weight_variable,
  ...
  def _get_cost(self, logits, cost_name, cost_kwargs):
    ...
    if cost_name == "cross_entropy":
      ...
      if class_weights is not None:
        ...
        loss_map = tf.nn.softmax_cross_entropy_with_logits(flat_logits,
                                                           flat_labels)
...
class Trainer(object):
  ...
  :param opt_kwargs: (optional) kwargs passed to the learning rate (momentum
  opt) and to the optimizer
  """
  ...
  def train(self, data_provider, output_path, training_iters=10, epochs=100,
    ...
    :param write_graph: Flag if the computation graph should be written as
    protobuf file to the output path
    """
    ...
    with tf.Session() as sess:
      ...
      for epoch in range(epochs):
        ...
        for step in range((epoch * training_iters),
          ...
          norm_gradients = [np.linalg.norm(gradient) for gradient in
                            avg_gradients]
